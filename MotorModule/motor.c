...
Timer_A_PWMConfig left_wheel_PWM_config = {
    ...
    TIMER_A_CLOCKSOURCE_DIVIDER_64,    // SMCLK / 64 = 38kHz, period = 1/38khz =
                                       // 0.026ns
    30000,                             // Tick Period
...
Timer_A_PWMConfig right_wheel_PWM_config = {
    TIMER_A_CLOCKSOURCE_SMCLK,         TIMER_A_CLOCKSOURCE_DIVIDER_64, 30000,
    TIMER_A_CAPTURECOMPARE_REGISTER_1, TIMER_A_OUTPUTMODE_RESET_SET,   0
};
...
Motor_init(void)
{
    ...
    GPIO_setAsPeripheralModuleFunctionOutputPin(
        GPIO_PORT_P2,
        GPIO_PIN4,
        GPIO_PRIMARY_MODULE_FUNCTION); // Configuring the peripheral module
                                       // function in the output direction for
                                       // P2.4 for primary module function modes
    ...
    GPIO_setAsPeripheralModuleFunctionOutputPin(
        ...
        GPIO_PRIMARY_MODULE_FUNCTION); // Configuring the peripheral module
                                       // function in the output direction for
                                       // P2.5 for primary module function mod
 ...
 */
static void
Motor_start(int left_duty_cycle, int right_duty_cycle)
{
...
Motor_stop(void)
{
    //    if (left_wheel_PWM_config.dutyCycle != 0 ||
...
Set_PID(void)
{
    ...
    g_pid_left += (error_left * KP) + (g_prev_error_left * KD)
                  + (g_sum_error_left * KI);
    g_pid_right += (error_right * KP) + (g_prev_error_right * KD)
                   + (g_sum_error_right * KI);
 ...
 */
void
TA3_0_IRQHandler(void)
{
    if (car.is_moving
        && (FORWARD == car.direction || BACKWARD == car.direction
            || STRAIGHT == car.direction))
        Set_PID();
